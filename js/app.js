...
colazo.config(['$routeProvider', function($routeProvider) {
    ...
    when('/addplace/:lat/:lng', {templateUrl: 'partials/addplace.html', controller: AddPlaceCtrl}).
...
function BiciSeguraCtrl($scope, $location) {
  ...
  function AddPlaceCtrl($scope, $http, $routeParams, $location, $window) {

    lat = $routeParams["lat"];
    lng = $routeParams["lng"];

    $scope.types = ["Taller", "Tienda", "Info", "Parqueadero", "Peligro"];


    $scope.add = function() {

      //var data = "place[title]="+$scope.name+"&place[description]="+$scope.description+"&place[longitude]="+lng+"&place[latitude]="+lat+"&place[category]="+$scope.type;
      var data = "place[title]="+$scope.name+"&place[description]="+$scope.description;

      $http.post("http://colazo.eu01.aws.af.cm/places", data).success(function() {
  ...
  function BiciMapaCtrl($scope, $http, $route, $location) {
    ...
    map.on('contextmenu', function(event) {
      ...
      //if (zoomlevel >= 17) {
        ...
        safeApply($scope, function() {
          $location.path("/addplace/"+lat+"/"+lng);
        });

      //}
      //else {
       
      //  navigator.notification.alert("Para tener una precisión mejor debe agrandar más el mapa", function() {}, "Debe agranadar más el mapa", "OK");
      //}
    ...
    if (connected) {
      ...
      $http.get('http://colazo.eu01.aws.af.cm/places.json').success(function(data, status, headers, config) {
        ...
        localStorage["places"] = angular.toJson(data);
      ...
      //if not connected
      var places = angular.fromJson(localStorage["places"]);
      ...
      for(i = 0; i < places.length; i++) {
        ...
        if (places[i].category.toUpperCase() == "INFO".toUpperCase()) {
          icon = icon_info;
        }
        else if (places[i].category.toUpperCase() == "TALLER".toUpperCase()) {
          icon = icon_repair;
        }
        else if (places[i].category.toUpperCase() == "PELIGRO".toUpperCase()) {
          icon = icon_theft;
        }
        else if (places[i].category.toUpperCase() == "PARQUEADERO".toUpperCase()) {
          icon = icon_parking;
        }
        else if (places[i].category.toUpperCase() == "TIENDA".toUpperCase()) {
          icon = icon_shop;
        }
        else {
          icon = L.Icon.Default;
      ...
      localStorage["places"] = angular.toJson(places);
